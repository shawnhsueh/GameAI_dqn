...
for e in range(EPISODES):
	...
	if e % 100 == 0:
		agent.save("../checkpoints/rand/BSG-dqn-beatrand_"+str(e)+".h5")
		print collections.Counter(wincounts[e/100-1])
win_rate = np.mean(wincounts, axis=1)
print(win_rate)
with open('../mkfig/win_rate.npy', 'wb') as f:
    np.save(f, win_rate)
round_mean = np.mean(counts, axis=1)
print(round_mean)
with open('../mkfig/round_mean.npy', 'wb') as f:
    np.save(f, round_mean)

optimal_steps = str(np.argmax(win_rate)*100)
print("optimal_steps", optimal_steps)
agent.load("../checkpoints/rand/BSG-dqn-beatrand_"+optimal_steps+".h5")
